...
//----------------------- 회원관리(member)
	...
	public List<MemberVO> memberListData(int page) {
		...
		try {
			...
			String sql = "SELECT id, name, email, phone, addr, num "
					...
					+ "FROM (SELECT id, name, email, phone, addr " + "FROM user_ WHERE admin ='n' ORDER BY id ASC)) "
	...
	public MemberVO memberDetailData(String id) {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT id,password, email, name, phone, postcode, addr, detail_addr, admin " + "FROM user_ "
					+ "WHERE id=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public int adEditProfile(MemberVO vo) {
		...
		try {
			// 수정
			conn = dbconn.getConnection();
			String sql = "UPDATE USER_ SET " + "name=?, " + "email=?, " + "phone=?, " + "postcode=?, " + "addr=?, "
					+ "detail_addr=?, " + "admin=? " + "WHERE id=?";
			ps = conn.prepareStatement(sql);
	...
	public int memberDeleteOk(String id) {
		...
		try {
			...
			sql = "DELETE FROM review_reply " + "WHERE id=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, id);
			ps.executeUpdate();
...
//----------------------- 특산물 관리(goods)
	/*
	 * //특산물 목록 (검색기능 추가) public List<GoodsVO> adGoodsListData(int page, String
	 * search){ List<GoodsVO> list = new ArrayList<GoodsVO>();
	 * 
	 * try { conn=dbconn.getConnection(); String
	 * sql="SELECT gno, gname, poster, price,manufacturer, num "
	 * +"FROM (SELECT gno, gname, poster, price,manufacturer, rownum as num "
	 * +"FROM (SELECT gno, gname, poster, price,manufacturer "
	 * +"FROM goods WHERE gname LIKE '%'||?||'%' ORDER BY gno ASC)) ";
	 * +"WHERE num BETWEEN ? AND ?"; ps=conn.prepareStatement(sql);
	 * 
	 * int rowSize=10; int start=(rowSize*page)-(rowSize-1); int end=rowSize*page;
	 * 
	 * ps.setString(1, search); ps.setInt(2, start); ps.setInt(3, end);
	 * 
	 * ResultSet rs=ps.executeQuery(); while(rs.next()) { GoodsVO vo=new GoodsVO();
	 * vo.setGno(rs.getInt(1)); vo.setGname(rs.getString(2));
	 * vo.setPoster(rs.getString(3)); vo.setPrice(rs.getString(4));
	 * vo.setManufacturer(rs.getString(5)); vo.setNum(rs.getInt(6)); list.add(vo); }
	...
	public List<GoodsVO> adGoodsListData(String search) {
		List<GoodsVO> list = new ArrayList<GoodsVO>();

		try {
			conn = dbconn.getConnection();
			String sql = "SELECT gno, gname, poster, price,manufacturer, num "
					+ "FROM (SELECT gno, gname, poster, price,manufacturer, rownum as num "
					+ "FROM (SELECT gno, gname, poster, price,manufacturer "
					+ "FROM goods WHERE gname LIKE '%'||?||'%' ORDER BY gno ASC)) ";
			/* +"WHERE num BETWEEN ? AND ?"; */
	...
	/*
	 * //특산물 총페이지 public int adGoodsTotalPage(String search) { int totalpage=0;
	 * 
	 * try { conn=dbconn.getConnection(); String
	 * sql="SELECT CEIL(COUNT(*)/10.0) FROM goods WHERE gname LIKE '%'||?||'%'";
	 * ps=conn.prepareStatement(sql); ps.setString(1, search); ResultSet
	 * rs=ps.executeQuery(); rs.next(); totalpage=rs.getInt(1); rs.close(); } catch
	 * (Exception e) { e.printStackTrace(); } finally { dbconn.disConnection(conn,
	 * ps); } return totalpage; }
	 */
	// 특산물 상세
	...
	public int adGoodsUpdate(GoodsVO vo) {
		...
		try {
			conn = dbconn.getConnection();
			conn.setAutoCommit(false);
			String sql = "UPDATE goods SET " + "gname=?,origin=?,manufacturer=?,price=? "
					+ "WHERE gno=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, vo.getGname());
			ps.setString(2, vo.getOrigin());
			ps.setString(3, vo.getManufacturer());
	...
	public int adGoodsDelete(int gno) {
		...
		try {
			conn = dbconn.getConnection();
			conn.setAutoCommit(false);
			String sql = "DELETE FROM cart WHERE gno=?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, gno);
			success = ps.executeUpdate();
...
//----------------------- 맛집 예약 관리(reservation)
	...
	public List<FoodReserveVO> adFdReserveList(int page) {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT f.no, f.title, r.day, r.time, r.inwon, r.ok, r.id,u.name,r.frno " + "FROM food f "
					+ "JOIN (SELECT res.*, ROWNUM AS num " + "FROM reservation res) " + "r ON f.no = r.fno "
					+ "JOIN user_ u ON r.id = u.id " + "WHERE r.num BETWEEN ? AND ?" + "ORDER BY r.frno DESC";
			ps = conn.prepareStatement(sql);
			int rowSize = 5;
	...
	public int adTotalresCount() {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT " + "(SELECT COUNT(*) FROM reservation WHERE ok = 0) + "
					+ "(SELECT COUNT(*) FROM reserve_Info WHERE rok = 0) AS combined_count " + "FROM dual";
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			rs.next();
			total = rs.getInt(1);
...
//----------------------- 전시회 예약 관리(reserveInfo)		
	...
	public List<ReserveInfoVO> adExReserveList(int page) {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT e.eno, e.ename, ri.day, ri.inwon, ri.rok, ri.id,u.name, ri.no  " + "FROM exhibition e "
					+ "JOIN (SELECT ri.*, ROWNUM AS num " + "FROM reserve_info ri) ri " + "ON e.eno = ri.eno "
					+ "JOIN user_ u ON ri.id = u.id " + "WHERE ri.num BETWEEN ? AND ? " + "ORDER BY ri.no DESC";
			ps = conn.prepareStatement(sql);
			int rowSize = 5;
			int start = (rowSize * page) - (rowSize - 1);
			int end = rowSize * page;
...
//--------------------- 문의내역 관리
	...
	public List<QnaBoardVO> adGetAllQnas(int page) {
		...
		try {
			...
			String sql = "SELECT qno,subject,regdate,status,name, id, num "
					...
					+ "FROM (SELECT qno,subject,regdate,status,name, id " + "FROM QnaBoard " + "WHERE group_step=0 "
					+ "ORDER BY qno DESC)) " + "WHERE num BETWEEN ? AND ?";
			ps = conn.prepareStatement(sql);
			int rowSize = 5;
			int start = (rowSize * page) - (rowSize - 1);
			int end = rowSize * page;
	...
	public int adQnasTotalPage() {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT CEIL(COUNT(*)/5.0) FROM QnaBoard " + "WHERE group_step=0";
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			rs.next();
			total = rs.getInt(1);
	...
	public int adQnasTotal() {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT COUNT(*) AS total FROM QnaBoard " + "WHERE group_step=0 AND status=0";
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			rs.next();
			total = rs.getInt(1);
	...
	public QnaBoardVO adQnaDetailData(int qno) {
		...
		try {
			conn = dbconn.getConnection();
			String sql = "SELECT qno,name,subject,cont,id,group_id " + "FROM qnaBoard " + "WHERE qno=" + qno;
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				vo.setQno(rs.getInt(1));
	...
	public void adQnaInsertData(QnaBoardVO vo) {
		try {
			...
			String sql = "INSERT INTO qnaBoard(qno,name,subject,cont,pwd,filename,filesize,id,group_id,"
					+ "group_step) " + "VALUES(qb_no_seq.nextval,?,?,?,?,?,?,?,?,?)";
			ps = conn.prepareStatement(sql);
			ps.setString(1, vo.getName());
			...
			sql = "UPDATE qnaBoard " + "SET status = 1 " + "WHERE group_id = " + vo.getGroup_id();
			ps = conn.prepareStatement(sql);
			ps.executeUpdate();
			ps.close();
		} catch (Exception e) {
